...
const currentCommittee = sessionStorage.getItem('loggedInCommittee') || '';
const committeeHasTimetable = sessionStorage.getItem('committeeHasTimetable') === 'true';


// ================= TIMETABLE DATA =================

let timetableData = typeof TIMETABLE_DATA !== 'undefined' ? TIMETABLE_DATA : {};
let committeeOCs = []; // OC names for current committee (from JSON)
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Load committee OC list from global COMMITTEE_DATA (loaded via <script> tag)
    if (typeof COMMITTEE_DATA !== 'undefined' && currentCommittee) {
        const myCommittee = COMMITTEE_DATA.find(c => c.name === currentCommittee);
        committeeOCs = myCommittee ? myCommittee.ocs.slice().sort() : [];
    }
    ...
    if (currentRole === 'cc') {
        ...
        // Hide timetable section if committee has no timetable data
        if (!committeeHasTimetable) {
...
function populateDropdown() {
    ...
    // Only show OCs from this committee that exist in timetable data
    const names = committeeHasTimetable
        ? Object.keys(timetableData).sort().filter(n => committeeOCs.length === 0 || committeeOCs.includes(n))
        : [];
    names.forEach(name => {
...
let ccFilter = 'all';
let ocFilter = 'all';
let workMode = 'immediate'; // 'immediate' | 'scheduled'
...
function findFreeOCs() {
    ...
    // Require all three fields before showing OC list
    if (!day || !startStr || !endStr) {
        container.innerHTML = '<div class="availability-helper">📋 Please select day and time range to check availability.</div>';
        return;
    }
...
function toggleOCSelection(name) {
    ...
    // Re-render the correct OC list based on mode
    if (workMode === 'immediate') {
        findFreeOCs();
    } else {
        populateScheduledOCs();
    }
}
...
function setWorkMode(mode) {
    ...
    document.getElementById('toggleImmediate').classList.toggle('active', mode === 'immediate');
    document.getElementById('toggleScheduled').classList.toggle('active', mode === 'scheduled');
    ...
    immediateSection.classList.toggle('section-hidden', mode !== 'immediate');
    scheduledSection.classList.toggle('section-hidden', mode !== 'scheduled');
    document.getElementById('deadlineRow').classList.toggle('section-hidden', mode !== 'scheduled');

    // Show/hide no-timetable message for immediate mode
    if (mode === 'immediate' && !committeeHasTimetable && noTimetableMsg) {
        noTimetableMsg.style.display = 'block';
        // Hide the day/time fields
        const modalRow = immediateSection.querySelector('.modal-row');
        if (modalRow) modalRow.style.display = 'none';
    } else if (noTimetableMsg) {
        noTimetableMsg.style.display = 'none';
        const modalRow = immediateSection.querySelector('.modal-row');
        if (modalRow) modalRow.style.display = '';
    }

    // Clear selections and tags
...
function populateScheduledOCs() {
    const container = document.getElementById('scheduledOCsList');
    const allOCs = committeeOCs.length > 0 ? committeeOCs : Object.keys(timetableData).sort();

    if (allOCs.length === 0) {
        container.innerHTML = '<div class="no-free-msg">No OCs found for this committee.</div>';
        return;
    }

    let html = '<div class="oc-checklist">';
    html += `<h4>👥 Select OCs (${allOCs.length} available) — choose one or more</h4>`;
    allOCs.forEach(name => {
        const isChecked = selectedOCs.includes(name);
        html += `<label class="oc-check-item ${isChecked ? 'checked' : ''}">
            <input type="checkbox" ${isChecked ? 'checked' : ''} onchange="toggleOCSelection('${name.replace(/'/g, "\\\'")}')">
...
function openTaskModal() {
    ...
    // Reset toggle to Immediate
    document.getElementById('toggleImmediate').classList.add('active');
    document.getElementById('toggleScheduled').classList.remove('active');
    document.getElementById('immediateSection').classList.remove('section-hidden');
    document.getElementById('scheduledSection').classList.add('section-hidden');
    document.getElementById('deadlineRow').classList.add('section-hidden');

    // Handle no-timetable state for immediate mode
    const noTimetableMsg = document.getElementById('noTimetableMsg');
    if (!committeeHasTimetable && noTimetableMsg) {
        noTimetableMsg.style.display = 'block';
        const modalRow = document.getElementById('immediateSection').querySelector('.modal-row');
        if (modalRow) modalRow.style.display = 'none';
    } else if (noTimetableMsg) {
        noTimetableMsg.style.display = 'none';
    }
    ...
    document.getElementById('freeOCsList').innerHTML = '';
    document.getElementById('scheduledOCsList').innerHTML = '';
    document.getElementById('selectedOCTags').innerHTML = '';
...
function assignTask() {
    ...
    // Build deadline string (only for scheduled)
    let deadline = '';
    let taskType = workMode;

    if (workMode === 'scheduled') {
...
function isTaskOverdue(task) {
    ...
    // Parse deadline: "YYYY-MM-DD" or "YYYY-MM-DD HH:MM"
...
function renderTaskCard(task, roleView) {
    ...
    if (isImmediate && task.status === 'pending') {
        typeBadge = '<span class="task-type-badge badge-immediate">⚡ Immediate</span>';
    } else if (overdue) {
        typeBadge = '<span class="task-type-badge badge-overdue">🔴 Overdue</span>';
    }

    // Deadline display
    let deadlineHtml = '';
    if (task.deadline) {
        const deadlineText = formatDeadlineDDMMYYYY(task.deadline);
        ...
        deadlineHtml = `
            <div class="task-meta-item">
                ...
                <div>
                    ...
                    <span class="meta-value ${deadlineClass}">${deadlineText}${overdueLabel}</span>
                </div>
            </div>`;
    }

    // Who field: CC sees "Assigned To", OC sees "Assigned By"
    const whoLabel = roleView === 'cc' ? 'Assigned To' : 'Assigned By';
    const whoValue = roleView === 'cc' ? task.assignedTo : task.assignedBy;
        ...
        <div class="task-card-body">
            ...
            <div class="task-meta-grid">
                <div class="task-meta-item">
                    <span class="meta-icon">👤</span>
                    <div>
                        <span class="meta-label">${whoLabel}</span>
                        <span class="meta-value ${whoClass}">${escapeHtml(whoValue)}</span>
                    </div>
                </div>
                <div class="task-meta-item">
                    <span class="meta-icon">📅</span>
            ...
            <div class="task-card-footer">
                ...
                ${(roleView === 'oc' && task.status === 'pending') ? `<button class="btn-complete" onclick="event.stopPropagation(); completeTask('${task.id}')">Mark Done</button>` : ''}
            </div>
        </div>
    </div>`;
}
...
function renderCCTasks() {
    ...
    let tasks = getTasks().filter(t => t.assignedBy === currentUser && (!currentCommittee || t.committee === currentCommittee || !t.committee));

    if (ccFilter !== 'all') {
        tasks = tasks.filter(t => t.status === ccFilter);
    }

    sortTasksPrioritized(tasks);
    ...
    // Build HTML with section dividers
    let html = '';
    ...
    if (immediateTasks.length > 0) {
        html += immediateTasks.map(t => renderTaskCard(t, 'cc')).join('');
        if (otherTasks.length > 0) {
            html += '<div class="task-section-divider">Scheduled Tasks</div>';
        }
    }
    html += otherTasks.map(t => renderTaskCard(t, 'cc')).join('');
...
function renderOCTasks() {
    ...
    let tasks = getTasks().filter(t =>
        ...
        (!currentCommittee || t.committee === currentCommittee || !t.committee)
    );
    ...
    if (immediateTasks.length > 0) {
        html += immediateTasks.map(t => renderTaskCard(t, 'oc')).join('');
        if (otherTasks.length > 0) {
            html += '<div class="task-section-divider">Scheduled Tasks</div>';
        }
    }
    html += otherTasks.map(t => renderTaskCard(t, 'oc')).join('');
